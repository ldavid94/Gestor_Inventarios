...
namespace CapaVista
{
    public partial class NEntrada : Form
    {
        ...
        public NEntrada(String texto)
        {
            InitializeComponent();
            llenarcbxNombreAlmacenes();
            llenarcbxNombreProveedores();
            actualizardvg();
            txtNombre.Text = texto;
            txtNombre.Enabled = false;
            txtRegistro.Text = "Entrada";
        ...
        public void llenarcbxNombreProveedores()
        {
            try
            {
                cbxProveedor.Items.Clear();
                OdbcDataReader datareader = sn.llenarcbxNombreP();
                while (datareader.Read())
                {
        ...
        public void llenarcbxNombreProducto()
        {
            try
            {
                cbxProducto.Items.Clear();
                OdbcDataReader datareader = sn.llenarcbxNProducto(cbxProveedor.Text);
                while (datareader.Read())
                {
        ...
        public void llenartxtStockEntradas()
        {
            try
            {
                txtEntrada.Items.Clear();
                OdbcDataReader datareader = sn.llenartxtEntrada(cbxIdProducto.Text);
                while (datareader.Read())
                {
                    txtEntrada.Items.Add(datareader[0].ToString());
                }
                txtEntrada.SelectedIndex = 0;
            }
            catch (Exception ex)
            {
        ...
        public void llenartxtIDProveedores()
        {
            try
            {
                cbxIdProveedores.Items.Clear();
                OdbcDataReader datareader = sn.llenarcbxIDprovee(cbxProveedor.Text);
                while (datareader.Read())
                {
                    cbxIdProveedores.Items.Add(datareader[0].ToString());
                }
                cbxIdProveedores.SelectedIndex = 0;
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error : " + ex);
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            sn.guardarEntrada(txtFecha.Text, txtNombre.Text, cbxIdProveedores.Text, cbxProveedor.Text, cbxIdProducto.Text, cbxProducto.Text, txtPrecioCompra.Text, txtPrecioVenta.Text, cantidad);
           

            int d1, d2, d3, total1, total2;

            d1 = (int)txtCantidad.Value;
            d2 = Convert.ToInt32(txtEntrada.Text);
            d3 = Convert.ToInt32(txtStock.Text);
            ...
            cmp = Convert.ToDouble(txtPrecioCompra.Text);
            ven = Convert.ToDouble(txtPrecioVenta.Text);
            ...
            sn.guardarEntradaInventario(entrada1, entrada2, cbxIdProducto.Text);
            actualizardvg();
            cleanTextbox();
        }
        ...
        private void btnRealizarEntrada_Click(object sender, EventArgs e)
        {
            ...
            paginahtml_texto = paginahtml_texto.Replace("@EMPRESA", cbxProveedor.Text);
            ...
            sn.relizarRegistroE(txtFecha.Text, txtHora.Text, txtRegistro.Text, txtNombre.Text, cbxProveedor.Text, total.ToString());
            sn.guardarBitacoraEntradas(txtFecha.Text, txtHora.Text, txtNombre.Text, total.ToString());
        }
        ...
        {
            llenarcbxNombreProducto();
            llenartxtIDProveedores();
            lblISProveedor.Text = cbxProveedor.Text;
        }

        private void lblISProveedor_Click(object sender, EventArgs e)
        {
            cbxProveedor.Select();
            SendKeys.Send("%{DOWN}");
        }
        ...
        private void txtPrecioCompra_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                MessageBox.Show("Solo Numeros con Decimal", "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                e.Handled = true;
                return;
            }
        }
        ...
        private void NEntrada_Paint(object sender, PaintEventArgs e)
        {
